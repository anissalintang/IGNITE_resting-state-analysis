...
calc_fcd () {
	...
	mkdir -p ${analysis_path}/ConnDensityTest
    fcd_path="${analysis_path}/ConnDensityTest"
    ...
    mkdir -p ${fcd_path}/wholeBrain/${s}/
    	...
    	3dDegreeCentrality \
    	-pearson \
    	-prefix ${fcd_path}/wholeBrain/${s}/${s}_wholeBrain_fcd.nii.gz \
    	-sparsity 1 \
    	-mask ${preproc_path}/${s}/meanFunc/bet/${s}_mean_func_bet_mask.nii.gz \
    	${vol_path}/temporalFiltering/filt_0.01-0.1/${s}/${s}_preprocessed_psc_filt01.nii.gz
